...
export default function CasoDetailPage() {
  ...
  const handleAddSeguimiento = async () => {
    ...
    setSegLoading(true);
    await createSeguimiento({ caso_id: caso.id, cliente_id: caso.cliente_id, lead_id: null, tipo: segTipo, descripcion: segDescripcion.trim() });
    ...
    <div className="space-y-4 md:space-y-6">
      <div>
        <div className="flex items-center gap-2 md:gap-4 mb-2">
          <Link href="/admin/casos" className="text-slate-500 hover:text-white transition-colors p-1"><ArrowLeft className="w-5 h-5" /></Link>
          <div className="flex-1 min-w-0">
            <h2 className="text-white text-base md:text-xl font-bold truncate">{caso.titulo}</h2>
            <p className="text-slate-500 text-xs md:text-sm truncate">{caso.cliente_nombre} — {caso.area}</p>
          </div>
          ...
          <div className="relative">
            ...
            {showShareModal && (
              <>
                ...
                <div className="absolute left-0 md:right-0 md:left-auto top-full mt-2 w-[calc(100vw-2rem)] max-w-72 bg-surface-dark border border-white/10 rounded-xl p-4 shadow-xl z-50 space-y-3">
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-lg p-2 flex items-center gap-2">
                    <input type="text" value={clientLink} readOnly className="flex-1 bg-transparent text-slate-400 text-xs outline-none min-w-0" />
                    <button onClick={handleCopyLink} className="text-accent hover:text-accent/80 transition-colors flex-shrink-0"><Copy className="w-4 h-4" /></button>
                  </div>
          ...
          {pipeline.stages.map((stage, i) => {
            ...
            return (
              <div key={stage.name} className="flex items-center flex-shrink-0">
                {i > 0 && <div className={`w-3 md:w-8 h-px mx-0.5 ${isPast || isCurrent ? "bg-accent" : "bg-white/10"}`} />}
                <div className={`flex items-center gap-0.5 md:gap-1 px-1.5 md:px-2 py-1 rounded-full text-[9px] md:text-xs font-medium transition-all ${
                  isPast ? "bg-green-500/15 text-green-400" : isCurrent ? "bg-accent/20 text-accent border border-accent/30" : "bg-white/5 text-slate-600"
                }`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
        <Card className="flex items-center gap-2 md:gap-3">
          ...
          <div className="min-w-0"><p className="text-slate-500 text-[10px]">Abogado</p><p className="text-white text-xs md:text-sm truncate">{caso.abogado}</p></div>
        </Card>
        <Card className="flex items-center gap-2 md:gap-3">
          <Scale className="w-4 h-4 text-accent flex-shrink-0" />
          <div className="min-w-0"><p className="text-slate-500 text-[10px]">Etapa</p><p className="text-white text-xs md:text-sm truncate">{caso.etapa}</p></div>
        </Card>
        <Card className="flex items-center gap-2 md:gap-3">
          <Clock className="w-4 h-4 text-accent flex-shrink-0" />
          <div><p className="text-slate-500 text-[10px]">En etapa</p><p className="text-white text-xs md:text-sm">{daysInStage}d</p></div>
        </Card>
        <Card className="flex items-center gap-2 md:gap-3">
          <CalendarClock className={`w-4 h-4 flex-shrink-0 ${deadlineDays !== null && deadlineDays <= 3 ? "text-red-400" : "text-accent"}`} />
          <div><p className="text-slate-500 text-[10px]">Deadline</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
        <Card>
          ...
          {caso.notas_etapa && (
            <div className="mt-3 pt-3 border-t border-white/10">
              <p className="text-slate-500 text-[10px] md:text-xs mb-1">Notas de etapa</p>
              <p className="text-slate-400 text-xs md:text-sm">{caso.notas_etapa}</p>
            </div>
          )}
        </Card>
        ...
        <Card>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowAddSeguimiento(!showAddSeguimiento)} className="flex items-center gap-1 text-accent text-xs hover:text-accent/80 transition-colors">
          ...
          {showAddSeguimiento && (
            <div className="mb-4 p-3 bg-white/5 border border-white/10 rounded-xl space-y-2.5">
                ...
                {TIPO_OPTIONS.map(({ value, label, icon: Icon }) => (
                  <button key={value} type="button" onClick={() => setSegTipo(value)}
                    className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-[11px] md:text-xs font-medium transition-colors ${
                      segTipo === value ? "bg-accent/15 text-accent border border-accent/30" : "bg-white/5 text-slate-500 hover:text-white"
                    }`}>
              ...
              <textarea value={segDescripcion} onChange={(e) => setSegDescripcion(e.target.value)} placeholder="Describe la actividad..." rows={2}
                className="w-full bg-white/5 border border-white/10 text-white text-xs md:text-sm px-3 py-2 rounded-lg placeholder-slate-600 focus:outline-none focus:border-accent/40 resize-none" />
              <div className="flex justify-end gap-2">
                <button onClick={() => { setShowAddSeguimiento(false); setSegDescripcion(""); }} className="text-slate-500 text-xs hover:text-white transition-colors px-3 py-1.5">Cancelar</button>
                <button onClick={handleAddSeguimiento} disabled={!segDescripcion.trim() || segLoading}
                  className="flex items-center gap-1.5 bg-accent text-surface-dark text-xs font-medium px-3 py-1.5 rounded-lg hover:bg-accent/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
          ...
          {!seguimientos?.length && !showAddSeguimiento ? (
            <p className="text-slate-500 text-xs md:text-sm">Sin actividad registrada</p>
          ) : (
            <div className="relative pl-5 md:pl-6 space-y-3 md:space-y-4">
              ...
              {seguimientos?.map((seg) => (
                <div key={seg.id} className="relative flex items-start gap-2 md:gap-3">
                  <div className="absolute -left-3.5 md:-left-4 top-1.5 w-2.5 md:w-3 h-2.5 md:h-3 rounded-full border-2 border-accent bg-surface-dark" />
                  <div className="p-1 md:p-1.5 rounded-lg bg-white/5 text-slate-500">{TIPO_ICONS[seg.tipo]}</div>
                  <div className="min-w-0 flex-1">
                    <p className="text-slate-300 text-xs md:text-sm">{seg.descripcion}</p>
                    <p className="text-slate-600 text-[10px] md:text-xs mt-0.5">
      ...
      <Card>
        ...
        <p className="text-slate-400 text-xs md:text-sm leading-relaxed">{caso.descripcion}</p>
      </Card>
    </div>
  );
}
