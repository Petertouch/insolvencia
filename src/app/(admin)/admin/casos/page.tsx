      ...
      <div className="flex items-center gap-3">
        <div className="flex bg-white/5 rounded-lg border border-white/10 p-0.5">
          <button
            ...
            className={`p-1.5 rounded-md transition-colors ${viewMode === "kanban" ? "bg-accent/15 text-accent" : "text-slate-500 hover:text-white"}`}
            title="Vista Kanban"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded-md transition-colors ${viewMode === "tabla" ? "bg-accent/15 text-accent" : "text-slate-500 hover:text-white"}`}
            title="Vista Tabla"
          >
        ...
        {viewMode === "tabla" && (
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 text-white text-sm pl-10 pr-4 py-2 rounded-lg placeholder-slate-600 focus:outline-none focus:border-accent/40"
            />
          </div>
        )}
      </div>
  ...
  return (
    <>
      <div className="md:hidden space-y-2">
        {casos.length === 0 ? (
          <p className="text-center text-slate-500 text-sm py-8">No se encontraron casos</p>
        ) : (
          casos.map((c) => (
            <Link key={c.id} href={`/admin/casos/${c.id}`}
              ...
              <div className="flex items-center gap-2 text-slate-500 text-xs">
                ...
                <span>-</span>
                <span>{c.etapa}</span>
                {c.fecha_limite && <span className="ml-auto text-slate-600">{new Date(c.fecha_limite).toLocaleDateString("es-CO", { day: "numeric", month: "short" })}</span>}
              </div>
            </Link>
          ))
        )}
              ...
              {casos.map((c) => (
                <tr key={c.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/admin/casos/${c.id}`} className="text-white hover:text-accent transition-colors font-medium">{c.titulo}</Link>
                    <p className="text-slate-500 text-xs mt-0.5">{c.area}</p>
                  </td>
                  <td className="px-4 py-3 text-slate-400">{c.cliente_nombre}</td>
                  <td className="px-4 py-3"><Badge variant="neutral">{c.etapa}</Badge></td>
                  <td className="px-4 py-3"><Badge>{c.prioridad}</Badge></td>
                  <td className="px-4 py-3 text-slate-400 hidden lg:table-cell">{c.abogado}</td>
