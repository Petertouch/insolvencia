    ...
    <div className="space-y-3 md:space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <span className="text-slate-500 text-xs md:text-sm">{leads?.length || 0} leads</span>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
        ...
        <div className="flex gap-2">
          <select value={fuenteFilter} onChange={(e) => setFuenteFilter(e.target.value)}
            className="flex-1 sm:flex-none bg-white/5 border border-white/10 text-white text-sm px-3 py-2.5 rounded-lg focus:outline-none focus:border-accent/40 appearance-none">
            <option value="" className="bg-surface-dark">Fuente</option>
            {["chatbot", "web", "referido", "whatsapp"].map((f) => (
              <option key={f} value={f} className="bg-surface-dark">{f}</option>
            ))}
          </select>
          <select value={estadoFilter} onChange={(e) => setEstadoFilter(e.target.value)}
            ...
            {["Nuevo", "Contactado", "Interesado", "Convertido", "Perdido"].map((e) => (
              <option key={e} value={e} className="bg-surface-dark">{e}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="md:hidden space-y-2">
        ...
        {leads?.length === 0 && <p className="text-center text-slate-500 text-sm py-8">Sin resultados</p>}
      </div>
              ...
              {leads?.map((l) => (
                <tr key={l.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/admin/leads/${l.id}`} className="text-white hover:text-accent transition-colors font-medium">{l.nombre}</Link>
                  </td>
                  <td className="px-4 py-3 text-slate-400">{l.telefono}</td>
                  <td className="px-4 py-3 text-slate-400 hidden lg:table-cell">{l.area_interes}</td>
