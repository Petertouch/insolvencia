...
export default function LeadDetailPage() {
  ...
  const handleConvert = () => { toast.success("Lead convertido a cliente (mock)"); };

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/admin/leads" className="text-slate-500 hover:text-white transition-colors"><ArrowLeft className="w-5 h-5" /></Link>
        <div className="flex-1">
          <h2 className="text-white text-xl font-bold">{lead.nombre}</h2>
          <p className="text-slate-500 text-sm">Lead — {lead.fuente}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="flex items-center gap-3"><Phone className="w-4 h-4 text-accent" /><div><p className="text-slate-500 text-xs">Telefono</p><p className="text-white text-sm">{lead.telefono}</p></div></Card>
        <Card className="flex items-center gap-3"><Mail className="w-4 h-4 text-accent" /><div><p className="text-slate-500 text-xs">Email</p><p className="text-white text-sm">{lead.email}</p></div></Card>
        <Card className="flex items-center gap-3"><MessageSquare className="w-4 h-4 text-accent" /><div><p className="text-slate-500 text-xs">Area de interes</p><p className="text-white text-sm">{lead.area_interes}</p></div></Card>
      </div>

      {lead.notas && (
        <Card><h3 className="text-white font-bold text-sm mb-2">Notas</h3><p className="text-slate-400 text-sm">{lead.notas}</p></Card>
      )}
