...
import { getSeguimientos, getCasos, createSeguimiento } from "@/lib/db";
...
export default function SeguimientoPage() {
  ...
  const handleSubmit = async () => {
    ...
    setFormDesc(""); setFormCasoId(""); setFormTipo("nota"); setShowForm(false); setFormLoading(false);
    refetch();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-slate-500 text-sm">{filteredSeguimientos?.length || 0} actividades</span>
        </div>
        <div className="flex items-center gap-2">
          <select value={tipoFilter} onChange={(e) => setTipoFilter(e.target.value)}
            ...
            <option value="" className="bg-surface-dark">Todos los tipos</option>
            {["llamada", "reunion", "whatsapp", "nota"].map((t) => (
              <option key={t} value={t} className="bg-surface-dark">{TIPO_LABELS[t]}</option>
            ))}
          </select>
      ...
      {showForm && (
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
            ...
            {TIPO_OPTIONS.map(({ value, label, icon: Icon }) => (
              <button key={value} type="button" onClick={() => setFormTipo(value)}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                  formTipo === value ? "bg-accent/15 text-accent border border-accent/30" : "bg-white/5 text-slate-500 hover:text-white hover:bg-white/10"
                }`}>
          ...
          <select value={formCasoId} onChange={(e) => setFormCasoId(e.target.value)}
            className="w-full bg-white/5 border border-white/10 text-white text-sm px-3 py-2.5 rounded-lg focus:outline-none focus:border-accent/40 appearance-none">
            <option value="" className="bg-surface-dark">Sin caso asociado</option>
            {casos?.filter((c) => c.etapa !== "Cerrado").map((c) => (
              <option key={c.id} value={c.id} className="bg-surface-dark">{c.titulo} — {c.cliente_nombre}</option>
            ))}
          </select>
          <textarea value={formDesc} onChange={(e) => setFormDesc(e.target.value)} placeholder="Describe la actividad..."
            rows={2} className="w-full bg-white/5 border border-white/10 text-white text-sm px-3 py-2 rounded-lg placeholder-slate-600 focus:outline-none focus:border-accent/40 resize-none" />
          <div className="flex justify-end">
            <button onClick={handleSubmit} disabled={!formDesc.trim() || formLoading}
              className="flex items-center gap-1.5 bg-accent text-surface-dark text-xs font-medium px-4 py-2 rounded-lg hover:bg-accent/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
      ...
      <div className="space-y-3">
        {filteredSeguimientos?.map((seg) => (
          <div key={seg.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-start gap-4 hover:bg-white/[0.07] transition-colors">
            <div className="p-2.5 rounded-xl bg-white/5 text-accent flex-shrink-0">{TIPO_ICONS[seg.tipo]}</div>
            <div className="flex-1 min-w-0">
              <p className="text-slate-300 text-sm">{seg.descripcion}</p>
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <Badge variant="neutral">{TIPO_LABELS[seg.tipo]}</Badge>
                {seg.cliente_nombre && (
                  <Link href={`/admin/clientes/${seg.cliente_id}`} className="text-accent text-xs hover:underline">{seg.cliente_nombre}</Link>
                )}
                {seg.lead_nombre && (
                  <Link href={`/admin/leads/${seg.lead_id}`} className="text-accent text-xs hover:underline">{seg.lead_nombre}</Link>
                )}
