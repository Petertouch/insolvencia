  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className={`grid grid-cols-2 ${isAdmin ? "md:grid-cols-4" : "md:grid-cols-3"} gap-2 md:gap-4`}>
        {isAdmin && (
          <StatCard title="Clientes" value={stats.totalClientes} icon={<Users className="w-5 h-5" />}
            trend={{ value: `${stats.clientesActivos} activos`, positive: true }} />
        )}
        ...
        <StatCard title={isAbogado ? "Estancados" : "Proc. Pend."}
          value={isAbogado ? stats.casosStale : stats.procesosPendientes}
          icon={<AlertTriangle className="w-5 h-5" />}
          trend={isAbogado ? { value: `${stats.casosDeadlineCerca} deadline`, positive: false } : { value: `${stats.procesosPendientes} por iniciar`, positive: false }} />
      </div>
              ...
              return (
                <Link key={c.id} href={`/admin/casos/${c.id}`}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-slate-500 text-[10px] md:text-xs truncate">{c.cliente_nombre} — {c.area}</p>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
            ...
            <div className="space-y-2.5 md:space-y-3">
              {stats.casosPorArea.map(([area, count]) => (
                <div key={area} className="flex items-center justify-between gap-2">
                  <span className="text-slate-400 text-xs md:text-sm truncate">{area}</span>
                  <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
                    <div className="w-20 md:w-32 bg-white/5 rounded-full h-1.5 md:h-2">
                      <div className="bg-accent h-full rounded-full" style={{ width: `${Math.min((count / Math.max(stats.casosAbiertos, 1)) * 100, 100)}%` }} />
                    </div>
        ...
        <Card>
          <div className="flex items-center justify-between mb-3 md:mb-4">
            ...
            <Link href="/admin/seguimiento" className="text-accent text-[10px] md:text-xs hover:underline">Ver todo</Link>
          </div>
          <div className="space-y-2.5 md:space-y-3">
            {actividad?.slice(0, 5).map((seg) => (
              <div key={seg.id} className="flex items-start gap-2.5 md:gap-3">
                <div className="p-1 md:p-1.5 rounded-lg bg-white/5 text-slate-500 mt-0.5 flex-shrink-0">{TIPO_ICONS[seg.tipo]}</div>
                <div className="min-w-0 flex-1">
                  <p className="text-slate-300 text-xs md:text-sm truncate">{seg.descripcion}</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="text-slate-500 text-[10px] md:text-xs truncate">{seg.cliente_nombre || seg.lead_nombre}</span>
                    <span className="text-slate-600 text-[10px] md:text-xs flex-shrink-0">
      ...
      <div className={`hidden md:grid ${isAdmin ? "grid-cols-4" : "grid-cols-2"} gap-3`}>
        {isAdmin && (
          <Link href="/admin/clientes/nuevo" className="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-accent/30 hover:bg-white/10 transition-all text-sm text-slate-400 hover:text-white">
            <Users className="w-4 h-4 text-accent" />Nuevo Cliente
          </Link>
        )}
        <Link href="/admin/casos/nuevo" className="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-accent/30 hover:bg-white/10 transition-all text-sm text-slate-400 hover:text-white">
          <Scale className="w-4 h-4 text-accent" />Nuevo Caso
        </Link>
        {isAdmin && (
          <Link href="/admin/leads/nuevo" className="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-accent/30 hover:bg-white/10 transition-all text-sm text-slate-400 hover:text-white">
            <Inbox className="w-4 h-4 text-accent" />Nuevo Lead
          </Link>
        )}
        <Link href="/admin/seguimiento" className="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:border-accent/30 hover:bg-white/10 transition-all text-sm text-slate-400 hover:text-white">
          <Phone className="w-4 h-4 text-accent" />Ver Seguimiento
        </Link>
      </div>
    </div>
  );
