  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-2xl p-5 md:p-8 space-y-4 md:space-y-5 w-full max-w-sm mx-4 md:mx-0">
      <div className="text-center mb-2">
        ...
        <p className="text-slate-500 text-sm mt-1">Panel administrativo</p>
      </div>

      <div>
        <label className="text-slate-400 text-xs font-medium mb-2 block">Ingresar como</label>
        <div className="space-y-2">
          {MOCK_USERS.map((u) => {
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${active ? "bg-accent/20" : "bg-white/10"}`}>
                  <Icon className={`w-4 h-4 ${active ? "text-accent" : "text-slate-500"}`} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className={`text-sm font-medium truncate ${active ? "text-white" : "text-slate-400"}`}>{u.nombre}</p>
                  <p className={`text-xs capitalize ${active ? "text-accent/70" : "text-slate-600"}`}>{u.role}</p>
                </div>
      ...
      <p className="text-center text-slate-600 text-xs">
        Acceso exclusivo para el equipo de Insolvencia 360
      </p>
    </form>
  );
}
